...
- `StatePortList.vue` can safely own agent real state port row markup and local scoped styles as long as `NodeCard.vue` keeps create-port popovers, state draft mutation, locked-edit guards, and graph mutation emits. Because `NodeCard.vue` uses scoped CSS, moving markup into a child requires moving the corresponding real-port row styles with it; otherwise child internals lose the port pill visual treatment.
