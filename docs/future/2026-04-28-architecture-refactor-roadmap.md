...
- `useCanvasConnectionInteraction.ts` 已承接 pending connection refs、preview point、auto-snapped target ref、active connection hover node ref、从 anchor 启停 pending connection、preview point 更新和 hover-change 通知；`EditorCanvas.vue` 仍保留 pointer-up routing、actual emits、panning、node drag/resize 和 DOM measurement。
...
- `canvasConnectionInteractionModel.ts` 已进一步承接高层 auto-snap target resolution 与 pending connection creation-menu request：flow hotspot、node body fallback、reverse state input source、state output target、create-input fallback、node creation payload 和打开菜单后的清理策略已可测试；`EditorCanvas.vue` 仍保留 DOM hit-test、pointer-to-canvas 坐标转换、actual emits、panning、node drag/resize 和 DOM measurement。
