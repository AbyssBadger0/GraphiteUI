...
test("applyEditorConfigToCanonicalGraph ignores legacy output state mirror fields and derives reads from the input port", () => {
  const graph: CanonicalGraphPayload = {
    graph_id: "graph_test",
    name: "Legacy Read Mirror Test",
    state_schema: {
      answer: {
        name: "Answer",
        description: "",
  ...
  const nextConfig = {
    ...
    fileNameTemplate: "",
    stateReads: [{ stateKey: "stale_answer", inputKey: "answer", required: true }],
  } satisfies OutputBoundaryNode & {
    stateReads: Array<{ stateKey: string; inputKey: string; required?: boolean }>;
  };

  const next = applyEditorConfigToCanonicalGraph(
    graph,
    {
      id: "output_answer",
      position: { x: 0, y: 0 },
...
test("applyEditorConfigToCanonicalGraph ignores legacy input state mirror fields and derives writes from the output port", () => {
  const graph: CanonicalGraphPayload = {
    graph_id: "graph_test",
    name: "Legacy Write Mirror Test",
    state_schema: {
      question: {
        name: "Question",
        description: "",
  ...
  const nextConfig = {
    ...
    value: "",
    stateWrites: [{ stateKey: "stale_question", outputKey: "question", mode: "replace" as const }],
  } satisfies InputBoundaryNode & {
    stateWrites: Array<{ stateKey: string; outputKey: string; mode?: "replace" }>;
  };
  ...
  assert.deepEqual(next.nodes.input_question.writes, [{ state: "question", mode: "replace" }]);
});
