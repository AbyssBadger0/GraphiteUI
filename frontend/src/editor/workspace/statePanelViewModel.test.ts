...
test("buildStatePanelViewModel returns sorted state rows with readable values", () => {
  ...
  assert.deepEqual(
    view.rows.map((row) => ({
      ...
      readers: row.readers.map((binding) => binding.nodeLabel),
      writers: row.writers.map((binding) => binding.nodeLabel),
    })),
    [
      {
        key: "alpha",
        ...
        readerCount: 1,
        writerCount: 1,
        readers: ["answer_helper"],
        writers: ["input_question"],
      },
      {
        key: "beta",
        title: "beta",
        ...
        readerCount: 1,
        writerCount: 1,
        readers: ["output_answer"],
        writers: ["answer_helper"],
      },
    ],
  );
