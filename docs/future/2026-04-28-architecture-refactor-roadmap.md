...
- `core/langgraph/runtime_setup.py` 已承接 LangGraph state schema/reducer construction、sentinel endpoint mapping、input boundary status marking、after-breakpoint passthrough callable setup、execution-edge index construction 和 after-breakpoint compile setup；`core/langgraph/runtime.py` 继续保留图构建、node/route callable、checkpoint、interrupt、cycle、progress persistence 和 output boundary execution。
...
- Phase 138 后 `core/langgraph/runtime.py` 从约 1,040 行降到 483 行；execution callables 和图构建主流程仍留在 runtime，checkpoint、interrupt、cycle、setup 和 progress helper 已迁出。Phase 138 的显式 import/call 让该文件比 Phase 137 多两行，但 after-breakpoint setup 职责已移出 runtime body。
