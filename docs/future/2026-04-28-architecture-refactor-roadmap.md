...
- `core/langgraph/runtime_setup.py` 已承接 LangGraph state schema/reducer construction、sentinel endpoint mapping、input boundary status marking、after-breakpoint passthrough callable setup 和 execution-edge index construction；`core/langgraph/runtime.py` 继续保留图构建、node/route callable、checkpoint、interrupt、cycle、progress persistence 和 output boundary execution。
...
- Phase 137 后 `core/langgraph/runtime.py` 从约 1,040 行降到 481 行；execution callables 和图构建主流程仍留在 runtime，checkpoint、interrupt、cycle、setup 和 progress helper 已迁出。
