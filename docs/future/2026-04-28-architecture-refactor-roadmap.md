...
- Phase 101 后 `node_system_executor.py` 从 1,226 行降到 882 行；执行主流程、state I/O、副作用、output artifact、streaming delta 和快照仍留在 executor，后续应继续优先抽取 state I/O 或 output artifact helper。
...
当前 P4 进展：`model_provider_client.py` 的共享 HTTP/request 层、provider discovery 层、OpenAI-compatible chat transport、Anthropic messages transport、Gemini generate-content transport、Codex responses transport 和共享 response parsing 已完成抽取；`node_system_executor.py` 的 condition evaluation、agent prompt、LLM output parser 和 execution graph 纯 helper 已完成抽取；LangGraph runtime 仍待迁移。
