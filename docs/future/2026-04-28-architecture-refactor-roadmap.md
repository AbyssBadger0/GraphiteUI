...
- `canvasNodeDragResizeModel.ts` 已承接节点拖拽/缩放 move 阶段的阈值判断、viewport-scale 投影、rounding、resize result projection，以及 node-resize pointer-down 的 missing-node / locked-edit / active-connection / start-resize 路由；`EditorCanvas.vue` 仍保留 pointer capture、animation-frame batching、connection completion、rendered-size lookup 和 graph mutation emits。
